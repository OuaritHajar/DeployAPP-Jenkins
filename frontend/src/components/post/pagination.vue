    ...
    <nav aria-label="pagination">
        <ul class="pagination">
            ...
            <div class="liste-pages row">
                <li class="page_item" v-for="(page, index) in nombreDePages" :key="index">
                    <button v-if="actuelPage == index +1" type="button" class="btn liste-pages__btn page-selected">
                        <span> {{ index +1 }} </span>
                    </button>
                    <button v-else type="button" class="btn liste-pages__btn" @click="goThisPage(index +1)">
                        <span> {{ index +1 }} </span>
                    </button>
                </li>
            </div>
...
export default {
    ...
    methods:{
        lastPage(){
            this.$router.push({ name: 'Posts', params: { page : JSON.parse(this.$route.params.page) - 1} })
        },
        nextPage(){
            console.log("page incrementé : ")
            this.$router.push({ name: 'Posts', params: { page : JSON.parse(this.$route.params.page) +1} })
        },
        goThisPage(pageId) {
